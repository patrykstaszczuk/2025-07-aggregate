...
def get_total_cost_pln_for_customer(session: Session, customer_id: UUID) -> float | None:
    ...
    currency_conversion = case(
        *_get_currencies_conversion(
            session=session,
            conditional_logic=Transaction.customer_id == customer_id,
        ),
    )
    return session.execute(
        select(
...
def get_total_income_for_product_in_pln(session: Session, product_id: UUID) -> float | None:
    ...
    currency_conversion = case(
        *_get_currencies_conversion(
            session=session,
            conditional_logic=Transaction.product_id == product_id,
        ),
    )
...
def _get_currencies_conversion(
    session: Session,
    conditional_logic: ColumnElement[bool],
) -> list[tuple[ColumnElement[bool], float]]:
    curr_converter = SimpleCurrencyRateToPlnProvider()
    currencies = session.execute(
        select(distinct(Transaction.currency)).where(conditional_logic),
    ).all()
    currency_conversion = []
